...
24. ✅ **Authority-intersection law p/ subagentes** (child authority = intersection(parent-delegation, child-def, context, policy)) — nenhum concorrente prova isso. **Concluído.** `Authority`/`Capability` (frozen, meet = greatest-lower-bound: caps ∩, allowed_tools ∩, deny ∪, can_spawn ∧) em `nullain/authority.py`; `RegisteredTool.requires` declarado por tool; `ToolRegistry` ganha `authority` + `scoped()` + `capability_surface()` e gating no `execute()` (deny absoluto, sem ASK); `AgentLoop.spawn` computa `effective = parent ∧ delegation(clamped) ∧ child_def ∧ policy`, projeta no policy do filho, e recusa `spawn` sem a capability SPAWN. Backward-compatible (`authority=None` = irrestrito). 18 testes provam a lei end-to-end (delegação READ-only bloqueia write mesmo com child_def+policy permitindo; clamp ao parent; depth cap; fator policy). Veja `docs/design/P4.24.md`.
25. ✅ **Plugin signing + SBOM + capability manifests.** **Concluído.** Plugin = bundle MCP assinado/manifestado: `PluginManifest` (transporte + capabilities + tool decls + SBOM pinados numa assinatura Ed25519 sobre serialização canônica, `nullain/plugins/manifest.py`); `SignatureVerifier` port + `Ed25519Verifier` (extra opcional `nullain-sdk[signing]`/`cryptography` via importlib) + `NoVerifier` fail-closed (`signing.py`); `PluginLoader` pipeline fail-closed — validate (schema/SBOM digest/requires⊆declared) → verify (signed+no backend=refuse; unsigned+require_signature=refuse) → capability-intersect `effective = declared ∩ operator_grant` (meet do P4.24 aplicado a plugins, tools over-grant dropped com log, all-dropped=refuse) → register (`mcp__<plugin>__<tool>` + requires/permission_level do manifest). Composição P4.23 sandbox + P4.24 authority (gate intersecta de novo em runtime). Wired no daemon (`_load_plugins` prepare-once/shared, skip-with-log per-plugin, register per-session) + `nullain.toml.example` `[plugins]` + `pyproject.toml` extra. 19 testes offline (fake verifier + FakeTransport) + 3 Ed25519 gated (roundtrip/tamper/unknown-key) + 6 daemon tests. Veja `docs/design/P4.25.md`.
26. ✅ **Tool search (schemas deferidos) p/ escalar MCP.** **Concluído.** `MCPClient.get_tool_schema` (fetch lazy de schema único, cache do `tools/list`); `RegisteredTool.schema_loader` + `hydrate_schema()`/`is_deferred`/`is_hydrated`; `ToolRegistry.search_tools(query)` (busca por nome+descrição, sem carregar schemas) + `hydrate_tool(name)` + `list_specs()` que **omite deferred não-hidratado** (o LLM só vê schema de tool que o agente escolheu usar); `register_mcp_tools(..., defer_schemas=True)` (opt-in; default eager, backward-compatible — plugin loader P4.25 inalterado); tool built-in `search_tools` (busca + hidrata matches, `nullain-tools/search.py`). 10 testes offline (FakeTransport). Veja `docs/design/P4.26.md`.
27. ✅ **Workflow orchestrator** (determinístico sobre subagentes) — equivalente ao `Workflow` do Claude. **Concluído.** DSL Python nativo (`nullain/workflow/`): `Workflow(name, description, fn)` + `WorkflowContext` com `agent()` (spawn via port `WorkflowSpawner`/`loop_spawner` sobre `AgentLoop.spawn`, autoridade P4.24), `parallel()` (barreira, thunk que lança ⇒ `None`), `pipeline()` (sem barreira entre stages, stage recebe `(prev, item, index)`, stage que lança dropa o item), `phase()`/`log()` (progresso). Eventos `WorkflowPhaseEvent`/`WorkflowLogEvent`/`WorkflowAgentEvent` no EventBus. Orquestração determinística (estrutura fixa pelo script; só os outputs dos subagentes variam). 12 testes offline (fake spawner). Veja `docs/design/P4.27.md`.
