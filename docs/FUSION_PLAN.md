...
- ✅ **Fase 5 — Limpeza final:** App — removido `agent/supervisor.py` (sem portar — ADR-3) e o `CircuitBreaker`/`retry_with_backoff` mortos de `agent/resilience.py` (mantido só `get_system_health`, chamado por `web/server.py`), removidas `build_wavespeed_tools`/`build_zuckpay_tools` (LangChain `StructuredTool`, substituídas por `agent/bridge_tools.py`). Nota: `agent/models.py` **não** é duplicação de `ModelRouter` — é um catálogo allow-list de seleção de modelo pra UI, já usado corretamente por `web/server.py`; não removido. `agent/mcp_client.py` já tinha sido removido na Fase 2. SDK — implementado `QuotaChecker` (ADR-4) como Protocol in-process consultado pelo `AgentLoop` (não via `HookManager`, que é subprocess-based e não tem lifecycle point pré-LLM-call — ver `nullain.quota.checker`).
