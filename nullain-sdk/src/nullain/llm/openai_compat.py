"""Nullain Agent SDK — OpenAI-Compatible Chat Completions LLM Provider Adapter.

Speaks the standard OpenAI ``/v1/chat/completions`` schema over a
configurable ``base_url`` and Bearer auth — this works unmodified against
OpenAI itself and any of the now-common OpenAI-compatible endpoints
(OpenRouter, Together, Groq, vLLM, LM Studio, and others) without a
provider-specific branch anywhere in this file. :class:`OllamaCloudProvider`
(``nullain.llm.ollama``) is a thin subclass of :class:`OpenAICompatibleProvider`
adding Ollama Cloud's own defaults and its optional native ``/api/chat``
endpoint — Ollama Cloud's default mode is this same v1-compatible schema, so
the split is defaults + one extra endpoint variant, not a different protocol
(issue #40).
"""
...
from nullain.llm.response_models import OpenAIResponse
...
logger: BoundLogger = get_logger("nullain.llm.openai_compat")
...
class OpenAICompatibleProvider(LLMProvider):
    ...
    Works against any endpoint implementing ``POST {base_url}/v1/chat/completions``
    with Bearer auth and the standard OpenAI request/response shape:
    OpenAI itself, OpenRouter, Together, Groq, vLLM, LM Studio, and any other
    OpenAI-compatible gateway. Only ``base_url`` (and, for a provider that
    requires it, ``api_key``) need to change between them — there is no
    provider-specific parsing branch in this class.
    """
    ...
    async def health_check(self) -> bool:
        """Check availability of the endpoint via ``GET /v1/models``."""
        client = self._client or httpx.AsyncClient(timeout=10.0)
        should_close = self._client is None
        try:
            response = await client.get(self._health_check_url(), headers=self._get_headers())
    ...
    def _handle_error_response(self, response: httpx.Response) -> None:
        ...
        text = response.text
        # A structured log line per non-200 response — otherwise this branch
        # only raised, with no observable trace of which status code came
        # back or why. Truncated so a large HTML/JSON error body doesn't
        # blow out a log line.
...
__all__ = ["OpenAICompatibleProvider", "TransientHttpError", "TransientTimeoutError"]
